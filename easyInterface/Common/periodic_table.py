...
from easyInterface.Utils.units import Mass, Length, FloatWithUnit, Unit, SUPPORTED_UNIT_NAMES
...
class SpeciesBase:
    ...
    def __init__(self, symbol: str,
        ...
        for key in kwargs.keys():
            if key not in Specie.supported_properties:
                raise ValueError("{} is not a supported property".format(key))
            else:
                self._properties[key] = kwargs[key]
                setattr(self.__class__, key,
                        property(lambda obj: obj._properties.get(key),
                                 lambda obj, value: obj._properties.__setitem__(key, value)))
    ...
    def __deepcopy__(self, memo):
        return self.__class__(self.symbol, self.oxi_state, **self._properties)
...
class Specie(SpeciesBase):
    ...
    def __init__(self, symbol: str,
        ...
        super().__init__(symbol, oxidation_state, **kwargs)
    ...
    def ionic_radius(self):
        ...
        if self._oxi_state in self._el.ionic_radii:
            return self._el.ionic_radii[self._oxi_state]
        d = self._el.data
    ...
    @classmethod
    def from_string(cls, species_string: str):
        """
    ...
    def get_crystal_field_spin(self, coordination: str = "oct",
        ...
        elec = self._el.full_electronic_structure
...
class DummySpecie(Specie):
    ...
    def __init__(self, symbol: str = "X", oxidation_state: float = 0, **kwargs):
        """
        Args:
